...
const CATEGORIES: Category[] = ['All', 'Environmental', 'Geotechnical', 'OccHyg'];
...
export default function InsightsFilterGrid({ insights }: { insights: Insight[] }) {
  ...
  return (
    <section className="insights-grid" aria-labelledby="insights-grid-heading">
      <div className="insights-grid__header">
        ...
        <nav className="insights-filter" aria-label="Filter articles by discipline">
      ...
      {filtered.length === 0 ? (
        <p className="insights-grid__empty">No articles in this category yet.</p>
      ) : (
